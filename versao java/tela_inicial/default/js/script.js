var uc = "TDSFLEXUC04"; // aqui vai a sigla do curso e o numero da uc
  ...
  $("li[data-item]").each(function () {
    ...
    if (local != null && local["uc"] == uc) {
      ...
      if (grupo == "lista-fazendo") {
        ...
        $(
          "li[data-item='" + local["item"] + "'] .dropdown-menu .mover-fazendo"
        ).hide();
        $(
          "li[data-item='" + local["item"] + "'] .dropdown-menu .mover-fazer"
        ).show();
      }
      if (grupo == "lista-feito") {
        $("li[data-item='" + local["item"] + "'").addClass("feito");
        $(
          "li[data-item='" + local["item"] + "'] .dropdown-menu .mover-feito"
        ).hide();
        $(
          "li[data-item='" + local["item"] + "'] .dropdown-menu .mover-fazer"
        ).show();
      }
      $("li[data-item='" + local["item"] + "'").prependTo(
        $("#" + local["grupo"] + "")
      );
    }
  });
...
$(".mover-fazer").click(function () {
  ...
  if (
    $(this).parent().parent().parent().parent().attr("data-index") ==
    "lista-fazendo"
  ) {
    id = parseInt(aux) - 1;
  } else if (
...
$(".link").click(function () {
  if ($(this).parent().parent().parent().attr("data-index") == "lista-fazer") {
    ...
    if (localStorage.getItem("verificaPrimeira") != true) {
...
dragula([
  ...
  .on("drag", function (el) {
    ...
    grupo =
      el.parentElement.parentElement.parentElement.getAttribute("data-group");
  ...
  .on("dragend", function (el) {
    ...
    if (
      ...
      $("li[data-item='" + el.getAttribute("data-item") + "'").prependTo(
        $("#" + retornaLista() + "")
      );
    }
  });
...
function verificaCompleto(lista) {
  if (
    $("div[data-group='" + lista + "'] ul[data-index='lista-fazer']").has("li")
      .length == 0 &&
    $("div[data-group='" + lista + "'] ul[data-index='lista-fazendo']").has(
      "li"
    ).length == 0
  ) {
